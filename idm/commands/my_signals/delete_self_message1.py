...
def delete_self_message(event: MySignalEvent) -> str:
    ...
    for mmsg in utils.get_all_history_gen(event.api, event.chat.peer_id):
        if datetime.now().timestamp() - mmsg['date'] > 3600:
            break
    ...
    try:
        ...
        message_id = utils.new_message(event.api, event.chat.peer_id, message="✅ Сообщения за час удалены")
