...
@dp.my_signal_event_handle('доверять')
def add_trusted_user(event: MySignalEvent) -> str:
    ...
    event.db.save()
    edit_message(event.api, event.chat.peer_id, event.msg['id'],
        message=f"✅ Пользователь [id{tr_user['id']}|{tr_user['first_name']} {tr_user['last_name']}] добавлен в доверенные.")
    return "ok"
...
@dp.my_signal_event_handle('доверенные', "доверненные")
