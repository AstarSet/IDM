...
## Как обновить (один хуй я что то обнавлю)
...
|Команда|Описание|
|---|---|
|!с пинг / пиу / кинг / соня / вадим /кыш |Отправляется смс с временем задержки|
|!с инфо / инфа / -i / info |Отправляется смс с информацией о дежурном и чате|
|!с д |Удаляет все сообщения за последнии 5 минут в чате|
|!с дд |Удаляет все сообщения за последний час в чате|
|!с ддд |Удаляет все сообщения за последнии 24 часа в чате|
...
|!с доверять [+ответ на сообщение] | Добавляет пользователя в список доверенных |
|!с -доверять [+ответ на сообщение] | Исключает пользователя из списока доверенных |
|!с доверенные | Выводит список доверенных пользоваетелей |

### Доступные когда вы дежурный в чате
|Команда|Описание|
|---|---|
|!д пинг / пиу / кинг / соня / вадим /кыш | Отправляется смс с временем задержки|
|!д инфо / инфа / -i / info |Отправляется смс с информацией о дежурном и чате|
|!д напиши[новая строка]Текст| Дежурный повторит текст (только для доверенных пользователей) |
